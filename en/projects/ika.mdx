...
Using the hardware and software foundation from the UGV project, the
robot can explore an indoor environment and build a map autonomously —
no human intervention required. All autonomous mapping works with
existing sensors (lidar + IMU), no extra hardware needed.

### 7.1. How It Works

The system consists of three layers: perception (SLAM + lidar),
navigation (Nav2 + obstacle avoidance), and exploration (frontier
algorithm).
...
flowchart TB
    ...
    AVOID -->|"cmd_vel"| MOTOR["Motor Driver"]
    GCS -->|"start/stop"| FRONTIER
```

Step-by-step: lidar scans → map forms → `frontier_explorer` detects
unknown boundaries → best frontier is sent to Nav2 as a goal → robot
navigates, map updates → cycle repeats → auto-stops at 99% coverage.

The algorithm scores frontiers with a utility function: large frontiers
are preferred, distant ones penalized, and repeated goals on the same
spot prevented. Detection interval, cluster size, and weights are
configurable via `frontier_explorer.yaml` (section 7.3).

### 7.2. Running

**In simulation** (no hardware needed):
...
The robot starts exploring `house.world` autonomously. Add
`enable_rviz:=true` for visual tracking.

<img src="/images/ika/gazebo-sim-screenshot.png" alt="Autonomous mapping in Gazebo simulation" />

**On real hardware**: the only difference is `use_sim_time:=false`:
...
**Exploration control** via ROS 2 services:

```bash
ros2 service call /exploration/start std_srvs/srv/Trigger    # start
ros2 service call /exploration/stop std_srvs/srv/Trigger     # stop
ros2 service call /exploration/save_map std_srvs/srv/Trigger # save map
```
...
Live map, teleop, and exploration controls appear on a single screen
at `http://localhost:8000`.

<img src="/images/ika/gcs-screenshot.png" alt="GCS screenshot" />

### 7.3. Configuration

Core parameters are in `gemstone_frontier_explorer/params/frontier_explorer.yaml`:

| Parameter | Default | Description |
|---|---|---|
| `detect_period` | 1.0 s | Detection interval |
| `cluster_min_cells` | 8 | Minimum frontier size |
| `w_size` | 1.0 | Size weight |
| `w_dist` | 2.0 | Distance weight |
| `commit_margin` | 0.05 m | Goal change prevention margin |
| `coverage_threshold` | 0.99 | Auto-termination threshold |
| `safety_distance` | 0.25 m | Safety distance |
